<template>
  <div class="chart-container">
    ...
    <canvas ref="SeaChart" v-if="!isLoading && !error"></canvas>
  </div>
</template>
...
const fetchSeaData = async () => {
  try {
    const response = await fetch(`${import.meta.env.BASE_URL}globalData/Global_Sea_Level_Rise_1993_2024.txt`);
    ...
    lines.forEach(line => {
      ...
      if (parts.length >= 2) {
        ...
        const sea = parseFloat(parts[4]);
        if (!isNaN(year) && !isNaN(sea)) {
          labels.push(year);
          values.push(sea);
        }
      }
    ...
    const sortedData = labels.map((year, index) => ({ year, sea: values[index] }))
      .sort((a, b) => a.year - b.year);

    const sortedLabels = sortedData.map(d => d.year);
    const sortedValues = sortedData.map(d => d.sea);
...
const createChart = (labels, data) => {
  ...
  chartInstance.value = new Chart(SeaChart.value, {
    type: "line",
    data: {
      labels: labels,
      datasets: [{
    ...
    options: {
      ...
      plugins: {
        ...
        title: {
          position: "top",
          align: "center",
          display: true, text: "全球海水面變化", color: '#fffff0', font: { size: 18, weight: 'bold', family: 'DM Sans, Noto Sans' } },
      ...
      scales: {
        ...
        y: {
          ticks: { font: { size: 12, weight: 'bold', family: 'DM Sans, Noto Sans' }, color: '#fffff0' },
          title: { display: true, text: "海水面高度 (mm)", color: '#fffff0', font: { size: 18, weight: 'bold', family: 'DM Sans, Noto Sans' } },
          grid: { color: "rgba(84, 84, 84, .8)", lineWidth: .4 },
        },
      }
    },
