<template>
  <div class="chart-container">
    ...
    <canvas ref="CO2Chart" v-if="!isLoading && !error"></canvas>
  </div>
</template>
...
const fetchCo2Data = async () => {
  try {
    const response = await fetch(`${import.meta.env.BASE_URL}globalData/co2_mm_mlo.txt`);
    ...
    const sortedData = labels.map((year, index) => ({ year, co2: values[index] }))
      .sort((a, b) => a.year - b.year);

    const sortedLabels = sortedData.map(d => d.year);
    const sortedValues = sortedData.map(d => d.co2);
...
const createChart = (labels, data) => {
  ...
  chartInstance.value = new Chart(CO2Chart.value, {
    type: "line",
    data: {
      labels: labels,
      datasets: [{
    ...
    options: {
      ...
      plugins: {
        ...
        title: {
          position: "top",
          align: "center",
          display: true, text: "全球 CO₂ 濃度變化", color: '#fffff0', font: { size: 18, weight: 'bold', family: 'DM Sans, Noto Sans' } },
      ...
      scales: {
        ...
        y: {
          ticks: { font: { size: 12, weight: 'bold', family: 'DM Sans, Noto Sans' }, color: '#fffff0' },
          title: { display: true, text: "CO₂ (ppm)", color: '#fffff0', font: { size: 18, weight: 'bold', family: 'DM Sans, Noto Sans' } },
          grid: { color: "rgba(84, 84, 84, .8)", lineWidth: .4 },
        },
      }
    },
