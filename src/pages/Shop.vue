<template>
    <header>
        <div class="header-wrapper">
            <div class="logo">
                <a href="#">
                    <img src="./image/logo.svg" alt="logo">
                </a>
            </div>
            <nav class="nav-large">
                <ul>
                    <li><a href="#">島嶼危機</a></li>
                    <li><a href="#">永續行動</a></li>
                    <li><a href="#">社群中心</a></li>
                    <li><a href="#">綠境方程式</a></li>
                    <li><a href="#">關於我們</a></li>
                    <li><a href="#">好站連結</a></li>
                </ul>
                <ul>
                    <li><a href="#"><i class="bi bi-handbag"></i></a></li>
                    <li><a href="#"><i class="bi bi-person-circle"></i></a></li>
                </ul>
            </nav>
        </div>


    </header>

    <div class="wrapper">
        <div class="banner">
            <img src="../assets/images/Sp01.png" alt="" class="banner_img">
            <div class="banner_h1">
                <h1>環保市集</h1>
            </div>
        </div>

        <div class="banner_text">
            <h4>首頁/商店</h4>
            <div class="store_title_S">
                <h4>商品排序</h4>
                <i class="fa-solid fa-caret-down"></i>
            </div>
        </div>

        <!-- 左側 篩選器  -->
        <div class="Sp_container_S">

            <div class="fliter_section_S">
                <ul>
                    <li><a href="#">NT$100~$200</a></li>
                    <li><a href="#">NT$200~$300</a></li>
                    <li><a href="#">NT$300~$400</a></li>
                    <li><a href="#">NT$400~$500</a></li>
                    <li><a href="#">NT$500 以上</a></li>
                </ul>
            </div>
            <!-- 商品列表 -->
            <ul class="product-grid_S">
                <!-- 商品1 -->
                <li class="product-card_S">
                    <div class="product-box_S">
                        <a href="#">
                            <img src="../assets/images/Sp04.jpg" alt="環保吸管1" class="product-img">
                        </a>
                        <button class="btn btn_S">加入購物車</button>
                    </div>
                    <a href="#">
                        <div class="bgr_words_1">
                            <p>環保吸管1</p>
                            <p>NT$150</p>
                        </div>
                    </a>
                </li>
                <!-- 商品2 -->
                <li class="product-card_S">
                    <div class="product-box_S">
                        <a href="">
                            <img src="../assets/images/Sp03.jpg" alt="環保吸管1" class="product-img">
                        </a>
                        <button class="btn btn_S">加入購物車</button>
                    </div>

                    <div class="bgr_words_2">
                        <a href="">
                            <p>環保吸管2</p>
                            <p>NT$160</p>
                        </a>
                    </div>
                </li>
                <!-- 商品3 -->
                <li class="product-card_S">
                    <div class="product-box_S">
                        <a href="">
                            <img src="../assets/images/Sp04.jpg" alt="環保吸管1" class="product-img">
                        </a>
                        <button class="btn btn_S">加入購物車</button>
                    </div>

                    <div class="bgr_words_2">
                        <a href="">
                            <p>環保吸管3</p>
                            <p>NT$200</p>
                        </a>
                    </div>
                </li>
                <!-- 商品4 -->
                <li class="product-card_S">
                    <div class="product-box_S">
                        <a href="">
                            <img src="../assets/images/Sp05.jpg" alt="環保吸管1" class="product-img">
                        </a>
                        <button class="btn btn_S">加入購物車</button>
                    </div>

                    <div class="bgr_words_2">
                        <a href="">
                            <p>環保袋子1</p>
                            <p>NT$250</p>
                        </a>
                    </div>
                </li>
                <!-- 商品5 -->
                <li class="product-card_S">
                    <div class="product-box_S">
                        <a href="">
                            <img src="../assets/images/Sp06.jpg" alt="環保吸管1" class="product-img">
                        </a>
                        <button class="btn btn_S">加入購物車</button>
                    </div>

                    <div class="bgr_words_2">
                        <a href="">
                            <p>環保袋子2</p>
                            <p>NT$500</p>
                        </a>
                    </div>
                </li>
                <!-- 商品6 -->
                <li class="product-card_S">
                    <div class="product-box_S">
                        <a href="">
                            <img src="../assets/images/Sp07.jpg" alt="環保吸管1" class="product-img">
                        </a>
                        <button class="btn btn_S">加入購物車</button>
                    </div>

                    <div class="bgr_words_2">
                        <a href="">
                            <p>環保袋子3</p>
                            <p>NT$195</p>
                        </a>
                    </div>
                </li>
                <!-- 商品7 -->
                <li class="product-card_S">
                    <div class="product-box_S">
                        <a href="">
                            <img src="../assets/images/Sp08.jpg" alt="環保吸管1" class="product-img">
                        </a>
                        <button class="btn btn_S">加入購物車</button>
                    </div>

                    <div class="bgr_words_2">
                        <a href="">
                            <p>環保杯子1</p>
                            <p>NT$310</p>
                        </a>
                    </div>
                </li>

                <!-- 商品8 -->
                <li class="product-card_S">
                    <div class="product-box_S">
                        <a href="">
                            <img src="../assets/images/Sp09.jpg" alt="環保吸管1" class="product-img">
                        </a>
                        <button class="btn btn_S">加入購物車</button>
                    </div>

                    <div class="bgr_words_2">
                        <a href="">
                            <p>環保杯子2</p>
                            <p>NT$200</p>
                        </a>
                    </div>
                </li>
                <!-- 商品9 -->
                <li class="product-card_S">
                    <div class="product-box_S">
                        <a href="">
                            <img src="../assets/images/Sp10.jpg" alt="環保吸管1" class="product-img">
                        </a>
                        <button class="btn btn_S">加入購物車</button>
                    </div>

                    <div class="bgr_words_2">
                        <a href="">
                            <p>環保杯子3</p>
                            <p>NT$450</p>
                        </a>
                    </div>
                </li>
                <!-- 商品10 -->
                <li class="product-card_S">
                    <div class="product-box_S">
                        <a href="">
                            <img src="../assets/images/Sp11.jpg" alt="環保吸管1" class="product-img">
                        </a>
                        <button class="btn btn_S">加入購物車</button>
                    </div>

                    <div class="bgr_words_2">
                        <a href="">
                            <p>環保筷子1</p>
                            <p>NT$280</p>
                        </a>
                    </div>
                </li>
                <!-- 商品11 -->
                <li class="product-card_S">
                    <div class="product-box_S">
                        <a href="">
                            <img src="../assets/images/Sp12.jpg" alt="環保吸管1" class="product-img">
                        </a>
                        <button class="btn btn_S">加入購物車</button>
                    </div>

                    <div class="bgr_words_2">
                        <a href="">
                            <p>環保筷子2</p>
                            <p>NT$200</p>
                        </a>
                    </div>
                </li>
                <!-- 商品12 -->
                <li class="product-card_S">
                    <div class="product-box_S">
                        <a href="">
                            <img src="../assets/images/Sp13.jpg" alt="環保吸管1" class="product-img">
                        </a>
                        <button class="btn btn_S">加入購物車</button>
                    </div>

                    <div class="bgr_words_2">
                        <a href="">
                            <p>環保筷子3</p>
                            <p>NT$320</p>
                        </a>
                    </div>
                </li>
            </ul>
        </div>
        <!-- 頁數選擇器 -->

        <div class="pages">
            <ul>
                <li>&lt;</li>
                <li><a href="#">1</a></li>
                <li><a href="#">2</a></li>
                <li><a href="#">3</a></li>
                <li><a href="#">4</a></li>
                <li><a href="#">5</a></li>
                <li>&gt;;</li>
            </ul>
        </div>
    </div>
    <!-- footer -->

    <footer>
        <div class="footer">
            <div class="footer-wrapper">
                <a href="#" class="logo-a"><img src="./image/logo_footer.svg" alt=""></a>
                <ul>
                    <li>
                        <a href="#">島嶼危機</a>
                        <a href="#">臺灣的響應</a>
                        <a href="#">教育推廣</a>
                        <a href="#">機智問答</a>
                    </li>
                    <li>
                        <a href="#">永續行動</a>
                        <a href="#">給地球的一封信</a>
                        <a href="#">今日減碳行動</a>
                    </li>
                    <li><a href="#">社群中心</a></li>
                    <li><a href="#">綠境方程式</a></li>
                    <li>
                        <a href="#">關於我們</a>
                        <a href="#">聯絡我們</a>
                    </li>
                    <li><a href="#">好站連結</a></li>
                    <li><a href="#">環保市集</a></li>
                    <li><a href="#">會員中心</a></li>
                </ul>
            </div>
        </div>
        <div class="copyright">
            <div class="copyright-wrapper">
                <p>&copy;本網站為緯育TibaMe【第94期】前端工程師專業技術養成班學員作品,僅供學習、展示之用途。</p>
                <ul>
                    <li><a href="#"><svg xmlns="http://www.w3.org/2000/svg" width="32" height="32"
                                viewBox="-2 -2 24 24">
                                <g fill="currentColor">
                                    <path
                                        d="M8.695 6.937v1.377H7.687v1.683h1.008V15h2.072V9.997h1.39s.131-.807.194-1.69h-1.576v-1.15c0-.173.226-.404.45-.404h1.128V5h-1.535C8.644 5 8.695 6.685 8.695 6.937" />
                                    <path
                                        d="M10 18a8 8 0 1 0 0-16a8 8 0 0 0 0 16m0 2C4.477 20 0 15.523 0 10S4.477 0 10 0s10 4.477 10 10s-4.477 10-10 10" />
                                </g>
                            </svg></a></li>
                    <li><a href="#"><svg xmlns="http://www.w3.org/2000/svg" width="32" height="32"
                                viewBox="-2 -2 24 24">
                                <g fill="currentColor">
                                    <path
                                        d="M10 18a8 8 0 1 0 0-16a8 8 0 0 0 0 16m0 2C4.477 20 0 15.523 0 10S4.477 0 10 0s10 4.477 10 10s-4.477 10-10 10" />
                                    <path
                                        d="M15 6.947c-.368.16-.763.27-1.178.318c.424-.25.748-.646.902-1.117a4.2 4.2 0 0 1-1.304.49A2.06 2.06 0 0 0 11.923 6c-1.133 0-2.051.905-2.051 2.02q0 .237.053.46a5.85 5.85 0 0 1-4.228-2.11a2 2 0 0 0-.278 1.015c0 .7.363 1.32.913 1.681a2.1 2.1 0 0 1-.93-.253v.025a2.03 2.03 0 0 0 1.646 1.98a2.1 2.1 0 0 1-.927.034a2.05 2.05 0 0 0 1.916 1.403a4.16 4.16 0 0 1-2.548.864q-.248 0-.489-.028A5.86 5.86 0 0 0 8.144 14c3.774 0 5.837-3.078 5.837-5.748l-.007-.262A4.1 4.1 0 0 0 15 6.947" />
                                </g>
                            </svg></a></li>
                    <li><a href="#"><svg xmlns="http://www.w3.org/2000/svg" width="32px" height="32px"
                                viewBox="0 0 24 24">
                                <path fill="#545454"
                                    d="M13.61 12.243a1.6 1.6 0 1 1-1.56-1.63a1.62 1.62 0 0 1 1.56 1.63" />
                                <path fill="#545454"
                                    d="M14.763 7.233H9.338a2.024 2.024 0 0 0-2.024 2.024v5.547a2.024 2.024 0 0 0 2.024 2.024h5.425a2.024 2.024 0 0 0 2.024-2.024V9.267a2.026 2.026 0 0 0-2.024-2.034m-2.713 7.723a2.703 2.703 0 1 1 2.642-2.703a2.67 2.67 0 0 1-2.642 2.703m2.936-5.405a.496.496 0 0 1-.496-.506a.506.506 0 1 1 1.012 0a.496.496 0 0 1-.557.506z" />
                                <path fill="#545454"
                                    d="M12.05 2a10 10 0 1 0-.1 20a10 10 0 0 0 .1-20m6.073 12.702a3.39 3.39 0 0 1-3.41 3.411H9.389a3.39 3.39 0 0 1-3.411-3.41V9.378a3.39 3.39 0 0 1 3.41-3.411h5.325a3.39 3.39 0 0 1 3.41 3.41z" />
                            </svg></a></li>
                </ul>
            </div>
        </div>
    </footer>
</template>

<script>
export default {

}
</script>
